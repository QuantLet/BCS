...
optimum = Rglpk_solve_LP(fct, st, leq,  
                         rhs, max=TRUE)
ptopt = c(optimum$solution[1],          
          optimum$solution[2],
          optimum$optimum)
...
wireframe(z ~ x*y, pts = pts, shade = F, aspect = 1,
          scales = list(col = "black", arrows = FALSE),
          drape = F, colorkey = F, screen = list(z = 70, x = -90, y = 30),
          par.settings = par.set, 
          xlab = list("x1", cex = 1.2),
          ylab = list("x2", cex = 1.2),
          zlab = list("f(x1,x2)", cex = 1.2),
          panel.3d.wireframe = function(x, y, z,xlim, ylim, zlim, 
                                        xlim.scaled, ylim.scaled, 
                                        zlim.scaled, pts,...){
            
            panel.3dscatter(x = pts[pts[, 3] <= 2*pts[, 1] + 4*pts[, 2], 1],
                            y = pts[pts[, 3] <= 2*pts[, 1] + 4*pts[, 2], 2],
                            z = pts[pts[, 3] <= 2*pts[, 1] + 4*pts[, 2], 3], 
                            xlim = xlim, ylim = ylim, zlim = zlim, 
                            xlim.scaled = xlim.scaled,
                            ylim.scaled = ylim.scaled, 
                            zlim.scaled = zlim.scaled,
                            type = "p", pch = 19, 
                            col = rgb(1, 0.05, 0.1, alpha = 0.5), 
                            cex = 0.1, .scale = TRUE, ...)
